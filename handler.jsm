  ...
  try {
    ...
    try {
      ...
      if (user) {
        ...
        if (!isNumber(user.coin)) user.coin = 15;
 ...
 *[ 🌱 ] Si consideras que esto es un error y cuentas con pruebas, puedes comunicarte con el propietario(a) del bot para apelar la suspensión.*`.trim();
        ...
        try {
          ...
          if (m.diamond) {
            m.reply('*[🌾] Se utilizaron ' + +m.diamond + ' diamante(s)*');
          }
        }
        break;
      }
  ...
  for (const nk of callUpdate) {
    if (nk.isGroup == false) {
      if (nk.status == 'offer') {
        ...
        const vcard = `BEGIN:VCARD\nVERSION:3.0\nN:;>\`\` I\'m Fz ~ 👑;;;\nFN:>\`\` I\'m Fz ~ 👑\nORG:>\`\` I\'m Fz ~ 👑\nTITLE:\nitem1.TEL;waid=522431268546:+52 243 126 8546\nitem1.X-ABLabel:>\`\` I'm Fz ~ 👑\nX-WA-BIZ-DESCRIPTION:[❗] ᴄᴏɴᴛᴀᴄᴛᴀ ᴀ ᴇsᴛᴇ ɴᴜᴍ ᴘᴀʀᴀ ᴄᴏsᴀs ɪᴍᴘᴏʀᴛᴀɴᴛᴇs.\nX-WA-BIZ-NAME:>\`\` I\'m Fz ~ 👑\nEND:VCARD`;
        await mconn.conn.sendMessage(nk.from, {contacts: {displayName: '>` I\'m Fz ~', contacts: [{vcard}]}}, {quoted: callmsg});
